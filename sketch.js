...
function HotelRoomScreen() {
  ...
  mirror.visible = true
  textSize(26)
  text("Click on the mirror to see the message.", width / 2, height / 2)
...
function BaseMentScreen() {
  ...
  text("Press Space to shoot the ghost....", width / 2, height / 2)

  if (keyDown("SPACE") && walk == "right") {
    bullet = createSprite(boy.x, boy.y, 20, 20)
    bullet.velocityX = 5;
...
function gameOverScreen() {
  ...
  textSize(40);
  if (state == "Win") {
    text("You Won the Gold !!!!", width / 2, height / 2);
  }
...
function spawnBats() {
  if (World.frameCount % 60 == 0) {
    var bat = createSprite(-10, random(0, height), 10, 10)
    bat.addImage(batImg)
    bat.scale = 0.2
    bat.velocityX = 5
    

    if (bat.isTouching(boy)) {
      gameState = "Over"
      state = "Loose"
